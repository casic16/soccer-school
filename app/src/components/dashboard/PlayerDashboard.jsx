  ...
  return (
    <div>
      ...
      {availabilities.length > 0 && (
        <>
          <h3 className="text-lg font-bold text-gray-700 mb-3">Mes disponibilités</h3>
            ...
            {availabilities.map((a) => (
              <div key={a.id} className="bg-white rounded-xl p-5 shadow-sm border border-yellow-200 flex justify-between items-center">
                ...
                <div className="flex gap-2">
                  <button onClick={() => updateStatus(a.id, 'confirmed')} className="px-3 py-1.5 bg-green-600 text-white text-sm rounded-lg hover:bg-green-700 transition">✓ Je viens</button>
                  <button onClick={() => updateStatus(a.id, 'maybe')} className="px-3 py-1.5 bg-yellow-100 text-yellow-600 text-sm rounded-lg hover:bg-yellow-200 transition">? Peut-être</button>
                  <button onClick={() => updateStatus(a.id, 'absent')} className="px-3 py-1.5 bg-red-100 text-red-600 text-sm rounded-lg hover:bg-red-200 transition">✗ Absent</button>
                </div>
              </div>
            ))}
          </div>
      ...
      <h3 className="text-lg font-bold text-gray-700 mb-3">Mon calendrier</h3>
